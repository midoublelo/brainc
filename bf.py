...
if __name__ == "__main__":
    ...
    if sys.argv[2] == "-opt":
        ...
        f = open(str(sys.argv[1])+".ll", "w")
        f.write(str(llvm_module))
        print("=== Written LLVM IR (Optimized) to file")
    elif sys.argv[2] == "-asm":
        # binding
        llvm.initialize()
        llvm.initialize_native_target()
        ...
        f = open(str(sys.argv[1])+".asm", "w")
        f.write(str(tm.emit_assembly(llvm_module)))
        print("=== Written assembly to file")
    elif sys.argv[2] == '-def':
        print("=== LLVM IR (Unoptimized)")
        print(module)
        f = open(str(sys.argv[1])+".ll", "w")
        f.write(str(module))
        print("=== Written LLVM IR to file")
    else:
        print("Must give compile type:")
        print("  -opt: optimized LLVM IR")
        print("  -def: unoptimized LLVM IR")
        print("  -asm: assembly code")